...
const MyRecipePage = () => {
    ...
    useEffect(() => {
        ...
        const getFoods = async () => {
            try {
                const response = await axios.get(import.meta.env.VITE_BACKEND_URL as string + `/api/food/getFoodByUserId`, {
    ...
    const handleDelete = async (recipeId: string) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(import.meta.env.VITE_BACKEND_URL as string + `/api/food/${recipeId}`, {
                        withCredentials: true
                    });
